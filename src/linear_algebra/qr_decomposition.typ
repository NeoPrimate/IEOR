...
= QR Decomposition <linear_algebra_qr_decomposition>
...
- $R$ is $n times n$ *upper #link(<linear_algebra_triangular_matrix>)[triangular]*

If $A$ is full column rank, the decomposition is unique up to signs on the diagonal of $R$.

For square $A$ (full rank), $Q$ is a full #link(<linear_algebra_orthogonal_matrix>)[orthogonal matrix] and $R$ is square upper triangular.

== Construction via Gram–Schmidt

The columns of $Q$ are the result of applying #link(<linear_algebra_gram_schmidt>)[Gram–Schmidt orthogonalization] to the columns of $A$:
...
Other uses:
- *#link(<linear_algebra_eigenvectors_eigenvalues>)[Eigenvalue]* computation via the QR algorithm — iteratively factor $A_k = Q_k R_k$ and update $A_(k+1) = R_k Q_k$; converges to a triangular matrix whose diagonal gives the eigenvalues
- *Orthonormalizing a basis* numerically (Gram–Schmidt is unstable in floating point; Householder QR is much more reliable)
- *Solving square systems* as an alternative to #link(<linear_algebra_lu_decomposition>)[LU]
...
- *#link(<linear_algebra_gram_schmidt>)[Gram–Schmidt Process]* — the orthogonalization underlying QR
- *#link(<linear_algebra_orthogonal_matrix>)[Orthogonal Matrix]*
- *#link(<linear_algebra_triangular_matrix>)[Triangular Matrix]*
- *#link(<linear_algebra_lu_decomposition>)[LU]* / *#link(<linear_algebra_cholesky_decomposition>)[Cholesky]* / *#link(<linear_algebra_svd>)[SVD]*
