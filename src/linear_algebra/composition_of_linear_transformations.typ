...
#align(center)[
  #frame(cetz.canvas({
    ...
    cetz.draw.content(
      (-1., 1.5),
      text(fill: purple)[$X subset.eq RR^n$],
    )
    cetz.draw.content(
      (7, 1.5),
      text(fill: green)[$Y subset.eq RR^m$],
    )
    cetz.draw.content(
      (12.2, 1.5),
      text(fill: red)[$Z subset.eq RR^l$],
    )
...
$
  T compose S(arrow(x) + arrow(y)) & = T(S(arrow(x) + arrow(y))) \
...
$
  T compose S (c arrow(x)) & = T(S(c arrow(x))) \
                           & = T(c S(arrow(x))) \
                           & = c T(S(arrow(x))) \
...
$
  T compose S (arrow(x)) & = T(S(arrow(x))) = T(A arrow(x)) \
...
#example[
  ...
  $
  $
    A B & = mat(A colorMath(vec(7, 9, 11), #green), A colorMath(vec(8, 10, 12), #purple);) \
        & = mat(
            colorMath(vec(1, 2, 3), #red) colorMath(vec(7, 9, 11), #green), , , colorMath(vec(1, 2, 3), #red) colorMath(vec(8, 10, 12), #purple)
            ; ;
            colorMath(vec(4, 5, 6), #blue) colorMath(vec(7, 9, 11), #green), , , colorMath(vec(4, 5, 6), #blue) colorMath(vec(8, 10, 12), #purple)
            ;
          )
  $
