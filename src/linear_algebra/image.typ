...
$
  T(L_0) & = {T(accent(x, arrow)_0 + t (accent(x, arrow)_1 - accent(x, arrow)_0)) | 0 lt.eq t lt.eq 1} \
...
#let transformation = frame(cetz.canvas(length: 6cm, {
  ...
  cetz.draw.content(
    "x0.end",
    text(green)[$T(accent(x, arrow)_0)$],
    anchor: "north-west",
    padding: 0.05,
    angle: "x0.end",
  )
...
$
  (c T)(accent(x, arrow)) & = c (T(accent(x, arrow))) \
