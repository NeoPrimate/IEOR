...
#let poisson_probs = k_vals.map(k => poisson.pmf(k, lambda_t))
#let max_poisson = calc.max(..poisson_probs)

// Generate time values for continuous plot
#let max_t = 6
#let t_vals = lq.linspace(0, max_t, num: 200)
#let exp_probs = t_vals.map(t => expon.pdf(t, rate: λ))
#let max_exp = calc.max(..exp_probs)

#let pois_dist = lq.diagram(
  width: 5cm,
  height: 5cm,
  xlim: (0, max_k + 1),
  ylim: (0, max_poisson * 1.1),
  xlabel: text(size: 0.75em)[Number of\ Arrivals ($k$)],
  ylabel: text(size: 0.75em)[Probability\ $P(X = k)$],
  yaxis: (tick-args: (tick-distance: max_poisson / 5)),
  lq.bar(k_vals, poisson_probs, fill: blue.lighten(75%), stroke: blue),
)

#let exp_dist = lq.diagram(
  width: 5cm,
  height: 5cm,
  xlim: (0, max_t),
  ylim: (0, max_exp * 1.1),
  xlabel: text(size: 0.75em)[Time\ Between\ Arrivals ($t$)],
  ylabel: text(size: 0.75em)[Probability\ Density $f(t)$],
  xaxis: (tick-args: (tick-distance: 1)),
  yaxis: (tick-args: (tick-distance: max_exp / 5)),
  lq.fill-between(t_vals, exp_probs, y2: t_vals.map(t => 0.0), fill: red.lighten(85%), stroke: none),
  lq.plot(t_vals, exp_probs, mark: none, stroke: red + 2pt),
)
