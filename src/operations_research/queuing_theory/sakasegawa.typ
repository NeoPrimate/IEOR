...
= Sakasegawa <operations_research_queuing_theory_sakasegawa>

Generalization of the #link(<operations_research_queuing_theory_kingman_vut>)[VUT formula] to *G/G/c* — multi-server queues with general arrival and service distributions.
...
Pooling reduces waiting time even at the same per-server utilization. This is the queueing analog of #link(<supply_chain_inventory_risk_pooling_risk_pooling>)[risk pooling].

== Limitations

- *Approximation*: tight in heavy traffic, less accurate when $rho$ is small
- *Identical servers* assumed
- *No abandonment*: customers can't leave; for that see #link(<operations_research_queuing_theory_mmc_abandonment>)[Erlang A]
...
- *#link(<operations_research_queuing_theory_kingman_vut>)[Kingman VUT]* — single-server case
- *#link(<operations_research_queuing_theory_linking_equation>)[Linking Equation]* — variance propagation across stations
- *#link(<operations_research_queuing_theory_erlang_c>)[Erlang C]* — exact M/M/c waiting probability
- *#link(<operations_research_queuing_theory_erlang_b>)[Erlang B]* — exact M/M/c/c (loss)
