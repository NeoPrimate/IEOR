...
= Factory Physics <supply_chain_manufacturing_factory_physics>
...
2. *#link(<operations_research_queuing_theory_kingman_vut>)[VUT equation]*: queue time = Variability × Utilization × Time. Reducing any of the three reduces flow time.

3. *#link(<supply_chain_manufacturing_best_worst_pwc>)[Best/Worst/Practical-worst-case]* performance curves: bound any manufacturing line's behavior given basic parameters.
...
See #link(<supply_chain_manufacturing_critical_wip>)[Critical WIP].
...
(See #link(<operations_research_queuing_theory_littles_law>)[Little's Law].)
...
Real lines lie between best (perfectly deterministic) and worst (max variability). PWC gives a "balanced random" baseline. See #link(<supply_chain_manufacturing_best_worst_pwc>)[Best/Worst/PWC].
...
3. *Right-size WIP*: just above critical WIP — see #link(<supply_chain_manufacturing_conwip>)[CONWIP]
4. *Pull system control*: #link(<supply_chain_manufacturing_kanban_sizing>)[kanban] / CONWIP / drum-buffer-rope
...
- *#link(<supply_chain_manufacturing_theory_of_constraints>)[Theory of Constraints]*
